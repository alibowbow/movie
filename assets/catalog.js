(() => {
  ...
  movies.forEach((movie,index)=>{
    const card=node('article','movie-card');
    ...
    const watch=node('span','watch','감상하기');preview.append(img,watch,node('span','duration',movie.duration));
    const content=node('div','card-content'),meta=node('div','card-meta');meta.append(node('span','',movie.genre),node('span','',movie.year));
    const heading=node('div','card-heading'),arrow=node('span','card-arrow','↗');arrow.setAttribute('aria-hidden','true');heading.append(node('h3','',movie.title));
